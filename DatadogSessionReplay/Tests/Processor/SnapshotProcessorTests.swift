...
class SnapshotProcessorTests: XCTestCase {
    ...
    func testWhenProcessingViewTreeSnapshot_itIncludeWebViewSlotFromNode() throws {
        ...
        let snapshot = ViewTreeSnapshot(
            date: time,
            context: .init(textAndInputPrivacy: .maskSensitiveInputs, touchPrivacy: .show, rumContext: rum, date: time),
    ...
    private func generateViewTreeSnapshot(for viewTree: UIView, date: Date, rumContext: RUMContext) -> ViewTreeSnapshot {
        snapshotBuilder.createSnapshot(of: viewTree, with: .init(textAndInputPrivacy: .maskSensitiveInputs, touchPrivacy: .show, rumContext: rumContext, date: date))
    }
